...
### active_reviews（review 排程持续化 — Entry + Position 双相位）

**alembic 0024（Phase B D1）** 加。**2026-05-15 round 4 D2 sync**：原本只用于持仓 review，现扩展为 Entry phase review loop 也复用此表。一行 = 一个 opp 的下次 review 时刻（按 `review_phase` 区分 entry / position 阶段）。APScheduler 每 ~10s 扫表 due 行，塞 `AGENT2_REVIEW_TICK` 任务，handler 内部按 `review_phase` 走不同路径（entry → 重新评估入场条件；position → 持仓 HOLD/PARTIAL_CLOSE/FULL_CLOSE/ADJUST_STOP 决策）。

| 字段 | 类型 | 说明 |
|---|---|---|
| `opportunity_id` | UUID FK UNIQUE | 关联 opp（CASCADE on opp delete） |
| `review_phase` | ENUM('ENTRY','POSITION') NOT NULL DEFAULT 'POSITION' | **2026-05-15 round 4 D2 加** — entry 阶段重评 vs 持仓阶段 review（待 D2 spec ship 后定 ENUM 实施细节：PG native ENUM type vs VARCHAR + CHECK） |
| `next_review_due_at` | TIMESTAMP TZ | 下次 review 时刻 |
| `review_interval_sec` | Integer | review 间隔（默认 300，事件熔断窗口缩到 30；entry phase 可能不同节奏，待 D2 spec 定） |
...
**Partial index**: `CREATE INDEX active_reviews_due_idx ON active_reviews (next_review_due_at) WHERE claimed_at IS NULL`（仅未认领行，scan 高效）。**2026-05-15 round 4 D2 sync**：可能加 `(review_phase, next_review_due_at) WHERE claimed_at IS NULL` 复合索引让 entry / position scan 分离，待 D2 spec ship 时决定。

**写入时机**：

| 时机 | 改 |
|---|---|
| 客户提交 opp + entry phase 启动（2026-05-15 round 4 D2 新增） | INSERT 一行 `review_phase='ENTRY'` + `next_review_due_at = now() + entry_review_interval` |
| Entry decision = ENTER（rejection_type=null） | UPDATE `review_phase='POSITION'`（同行复用，由 entry loop 切到 position loop） |
| Entry decision = REJECT + rejection_type='TEMPORARY' | UPDATE `next_review_due_at = now() + entry_review_interval`（继续 entry loop） |
| Entry decision = REJECT + rejection_type='PERMANENT' | DELETE（永久放弃，opp 走 FAILED 终态） |
| `ENTRY_FILLED` handler（旧路径，position 入场后） | INSERT/UPDATE `review_phase='POSITION'`, `next_review_due_at = now() + 5 min` |
...
> **2026-05-15 round 4 sync**: D2 决策 — Entry phase review loop 复用此表，待 D2 spec ship 后定 schema 细节（ENUM 实施 + 复合索引 + entry_review_interval 默认值 + entry phase exit 状态转换图）。
...
| Bundle | `bundle_id` / `bundle_json` | String(64) UNIQUE / JSONB | bundle 输入 snapshot（**2026-05-15 round 4 D5 sync**：BundleV1 10 维 → BundleV2 12 维，加 dim 11 长线指标 21 项 + dim 12 事件影响。`bundle_json` JSONB schema 自然向后兼容，无 schema 变更，但读取代码须按 `bundle_version` 字段区分 V1/V2，待 D5 spec ship 后定 version 字段位置） |
...
| Entry decision | `rejection_type` | String(16) NULL | **2026-05-15 round 4 D2 加** — entry phase REJECT 原因：TEMPORARY（条件不满足，下次 review 重试）/ PERMANENT（永久放弃，opp → FAILED）/ null（非 entry phase 或 ENTER 决策）。可能也 inline 进 `parsed_decision_json` 而非独立列，待 D2 spec ship 后定 |
...
> **2026-05-15 round 4 sync**：D5 BundleV2 12 维 + D2 `rejection_type` 字段实施细节（独立列 vs JSONB inline）待 D2/D5 spec ship 后定。
...
### event_calendar（事件熔断窗口数据源 + D6 dim 12 关联源）

**alembic 0027（Phase B D4）** 加。一行 = 一个已知事件。本期 source 全 MANUAL（用户/Claude 手录），远景 §4 #3 EventCalendarCollector ship 时加 AUTO_THIRD_PARTY。**2026-05-15 round 4 D6 sync**：此表数据将作为 dim 12（事件影响）的输入源之一 — Agent 3 读 `event_calendar` 拉对应 symbol 的近期事件（earnings/FOMC/CPI 等），结合新闻摘要生成 ~500 字 dim 12 事件影响摘要写入 BundleV2。
...
**2026-05-15 round 4 D6 sync — Agent 2 现在感知事件**（原"本期不感知事件"已废止）：BundleV2 dim 12 = 事件影响摘要（Agent 3 写入 ~500 字），含 `event_calendar` 拉来的 macro events（earnings/FOMC/CPI dates）+ 影响评估。系统层 active_reviews 加快频率仍保留，但 Agent 2 现在 explicitly 读 dim 12 自决是否提前平仓 / 调整 SL（不再仅依赖 dim 4 期权链 + dim 8 OI 间接推断事件）。待 D6 spec ship 后定 dim 12 schema 细节（inline JSONB vs 独立 `event_impact_snapshots` per-symbol 缓存表）。
...
| `agent2_decisions` | `bundle_json` / `raw_response_json` / `parsed_decision_json` / `order_ids` | Agent 2 决策完整 trace（**2026-05-15 round 4 D5 sync**：`bundle_json` schema 由 BundleV1 10 维 升级到 BundleV2 12 维，加 dim 11 长线指标 21 项 + dim 12 事件影响 ~500 字摘要；JSONB 类型不改，schema 演进通过 `bundle_version` 字段标识） |
...
### 2026-05-15 round 4 客户开会 6 项变更 — 待加表提议（spec ship 时定细节）

| 表 | 关联决策 | 用途 | 实施方向 |
|---|---|---|---|
| `long_term_stats_cache` | **D5 BundleV2 dim 11** | per-symbol 缓存 21 项长线指标（~600 tokens），避免每次 review 重算 | inline JSONB 进 BundleV2 `bundle_json` 已可满足；独立表收益 = 跨 opp 共享 cache + TTL 控制；待 D5 spec ship 时定 inline vs 独立表 |
| `event_impact_snapshots` | **D6 BundleV2 dim 12** | per-symbol 缓存 Agent 3 生成的 ~500 字事件影响摘要 | 同上，待 D6 spec ship 时定 inline JSONB（进 `bundle_json`）vs 独立表（跨 opp 共享 + invalidate by `event_calendar` update） |
| `chaos_test_runs` 或 `failure_events` | **D4 16 类故障矩阵** | 记录每次混沌测试 / 真实故障的事件流（IBKR 断连 / Pool 降级 / Agent timeout / DB 锁等 16 类） | 跟 `pool_health_log` 互补：pool_health 只记 4 Pool 状态切换，chaos_test_runs 覆盖更广故障矩阵 + 自动化测试触发记录；待 D4 spec ship 时定 |
| `daily_summary_reports` | **D4 每日总结** | 每日 close 后生成系统级报告（开仓数 / 成交数 / PnL / 故障 / Agent 决策审计聚合） | 可能 view（materialized）替代实表；待 D4 spec ship 时定 |

> **2026-05-15 round 4 sync 原则**：上述 4 张提议表 schema **均不在本期 ship**，等 D4/D5/D6 spec ship 时正式立表。文档先记录设计意图防遗忘。
