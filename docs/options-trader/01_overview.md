...
- 触发条件（立即 / 时间 / PRICE_BREACH 价格突破 / 单 MA MA_CROSSOVER / 时间+条件，**v1 永久范围**, 2026-05-15 round 4 sync — D1）
...
| 3 | **Strategy Run（策略运行）** | 一次 Agent 2 决策（输入 12 维 BundleV2，输出 StrategyDecision + 风险门验证）(2026-05-15 round 4 sync) |
...
| 6 | **Task Queue（任务队列）** | `workflow_tasks` 表驱动的 9 种 event_type 任务调度 (2026-05-15 round 4 sync) |
...
| 2 | **9 个 event_type handler 函数** (2026-05-15 round 4 sync) | 取任务 → 处理 → 可能塞新任务进队列 | 各 handler 取自己 event_type 的任务 |
...
### 9 种 event_type 一览（任务总分类，2026-05-15 round 4 sync）
...
| 9 | `AGENT3_EVENT_REFRESH_TICK(symbol)` (2026-05-15 round 4 sync) | APScheduler (盘前 / 事件公布后 / 每日刷新) | Agent 3 Event Refresh → 更新 BundleV2 dim 11 长线指标 + dim 12 事件影响 |
...
graph TB
    ...
    subgraph LLM 层
        ...
        A2["Agent 2 = Strategy<br/>entry: DeepSeek V4-Flash<br/>review: Claude Haiku 4.5<br/>12 维 BundleV2 (2026-05-15)"]
        A3["Agent 3 = Event Refresh<br/>(2026-05-15 round 4)<br/>更新 dim 11 长线 + dim 12 事件"]
    end

    subgraph 任务调度核心
        APS["APScheduler<br/>+ pandas_market_calendars<br/>+ ZoneInfo America/New_York"]
        WQ[("workflow_tasks<br/>任务队列<br/>9 种 event_type (2026-05-15)")]
        AR[("active_reviews<br/>review_phase ENUM<br/>entry loop + 5 min review")]
    end
...
## 三个 AI Agent（2026-05-15 round 4 sync — Agent 3 加入）

整个系统**只有三个 LLM Agent**（Agent 1 Intake / Agent 2 Strategy / Agent 3 Event Refresh），全部走 DeepSeek V4-Flash（Anthropic-compat 端点；模型选型详见 [[依赖项与配置|08_dependencies.md]]）。Risk Gate **不是 AI**，是确定性查表代码。
...
| `trigger_rule` | 立即 / 时间 / PRICE_BREACH / 单 MA MA_CROSSOVER / 时间+条件（**v1 永久范围**, 2026-05-15 round 4 sync — D1） | 不在范围 → validation_errors_zh 阻塞 |
...
| 工作点 | 触发 | LLM 模型 | 输入 | 输出 |
|---|---|---|---|---|
| **(A) 入场决策** | `CONDITION_MET` event | DeepSeek V4-Flash | 12 维 BundleV2 (entry 时维度 5 持仓盈亏 + 维度 10 上次 summary 为 null；维度 11/12 由 Agent 3 注入) (2026-05-15) | 策略 + 行权价 + 到期 + 手数 + legs |
| **(B) 持续 review** | `AGENT2_REVIEW_TICK` event | Claude Haiku 4.5 (固定不升 Sonnet 省成本) | 12 维 BundleV2 (齐全) (2026-05-15) | HOLD / PARTIAL_CLOSE / FULL_CLOSE + ≤600 字 summary |

**Entry phase review loop（2026-05-15 round 4 sync — D2）**：
- `active_reviews` 表加 `review_phase` ENUM（entry / hold / event-burst）+ `rejection_type` 列
- Entry 决策被风险门拒绝后，进入连续 loop（LLM 重读 BundleV2 + rejection 反馈再次产策略），单次 timeout = 60s
- 详细 schema / 状态机详见 [[Agent2：策略决策器|04_strategy.md]] + [[数据库与持久化|06_database.md]]
...
### Agent 3 = Event Refresh（长线指标 + 事件影响刷新，2026-05-15 round 4 sync — D6）

**职责**：为 BundleV2 提供 dim 11 长线指标 (21 项, e.g. 季度营收 / 长线 MA / 估值倍数 / 板块相对强度等) + dim 12 事件影响 (earnings / 宏观 / 政策事件对持仓的定性评估)。

**触发**：APScheduler 排 `AGENT3_EVENT_REFRESH_TICK(symbol)` 任务 — 盘前 / 事件公布后 / 每日刷新。Agent 2 entry + review 时**只读 BundleV2 cache**，不实时调 Agent 3（解耦 latency + cost）。

**与 Agent 2 的关系**：sibling（不嵌套）— Agent 2 决策时 BundleV2 是输入，dim 11/12 由 Agent 3 异步预先注入。

详细 dim 11/12 schema 见 [[Agent2：策略决策器|04_strategy.md]]。

Sources: [strategy_agent.py:1-129](https://github.com/ChunmiaoYu/options_ai_trader/blob/6b3d159/src/options_event_trader/agents/strategy_agent.py#L1-L129), invariant 5 (v3), invariant 10, invariant 20, [feedback_agent1_role_dispatch_not_validate memory], [project_vision_and_north_star §1, §5.1.1, §11, §16](specs/architecture-walkthrough.md), [north-star-v1-target.md](specs/north-star-v1-target.md)
...
| **WorkflowEventType enum + EVENT_HANDLERS dispatch** | ✅ | Phase B `ffd04e3` ship 8 新 + 3 legacy event_type (Agent 3 第 9 个 event_type AGENT3_EVENT_REFRESH_TICK 待 ship, 2026-05-15 round 4 sync) |
...
| P0 主线 | 工作量估 | 说明 |
|---|---|---|
| **9 个 event_type handler 真实施**（`F-2026-05-07-WORKER-HANDLER-IMPLEMENTATION`，含 Agent 3 第 9 handler, 2026-05-15 round 4 sync） | ~1-2 天 | 当前 worker 仍是单进程 `while True` 4 步循环（[loop.py:132-234](https://github.com/ChunmiaoYu/options_ai_trader/blob/6b3d159/src/options_event_trader/worker/loop.py#L132-L234)），未升级为 9 handler 取任务驱动模型 |
| **Agent 3 Event Refresh + BundleV2 dim 11/12 真实施** (2026-05-15 round 4 sync — D6 + D5) | TBD | 长线 21 项指标 + 事件影响刷新；BundleV2 cache 写入逻辑 |
| **Entry phase review loop**（active_reviews 加 review_phase ENUM + rejection_type + 60s timeout, 2026-05-15 round 4 sync — D2） | TBD | schema migration + entry loop 状态机 |
| **16 类故障矩阵 + 告警 3 类邮件** (2026-05-15 round 4 sync — D4) | TBD | 全覆盖告警链路 |
| **AI 决策时间线 tab** (2026-05-15 round 4 sync — D3) | TBD | 前端 Strategy Tab 加 AI 决策时间线视图 |
...
> **下次大架构改动同步规则**：改 9 event_type / 4 Pool/Client / DB schema / APScheduler 调度逻辑时，**回到本文档 + [02_architecture.md](02_architecture.md) + [架构通俗讲解](specs/architecture-walkthrough.md) + 项目内北极星 memory** 四处同步更新。(2026-05-15 round 4 sync)
...
| **USER_B**（live 账户 程序 user） | live | PROD IB Gateway 独占；跑 4 个 Pool/Client + 9 handler + 下单 (2026-05-15) | PROD VM |
...
| 文档 | 内容 |
|---|---|
| [[系统架构\|02_architecture.md]] | 4 大组成部分深度展开：APScheduler / 9 event_type handler (含 Agent 3, 2026-05-15) / 4 Pool/Client / 任务队列；端到端数据流时序图；并发与崩溃恢复 |
| [[Agent1：Intake 解析器\|03_intake.md]] | LangGraph 节点 / Pydantic schema / DeepSeek prompt / validation 阻塞机制 / 中文公司名映射 |
| [[Agent2：策略决策器\|04_strategy.md]] | 12 维 BundleV2 构建（含 Agent 3 注入 dim 11/12, 2026-05-15）/ entry vs review 双工作点 / 风险门 2 层 / Adaptive review interval / 事件熔断窗口 / Entry phase review loop |
