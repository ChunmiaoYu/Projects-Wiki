...
1. **决策 → 执行的最后一公里**：[worker/agent2_scheduler.py:114-127](../../src/options_event_trader/worker/agent2_scheduler.py#L114-L127) `review_fn` 拿到 orchestrator 返回的 trace **直接 return 扔掉**，本 spec 让它路由到 `exit_order_placer`（PARTIAL/FULL_CLOSE）或 UPDATE 新 `position_adjustments` 表（ADJUST_STOP）
2. **决策持久化**：`agent2_decisions` DB 表永远空（ORM model 在 [db/models.py:380](../../src/options_event_trader/db/models.py#L380)，无 INSERT 代码）→ 本 spec 加 `decision_repository`
...
**表结构**（已有 ORM [db/models.py:380](../../src/options_event_trader/db/models.py#L380)，本 spec 不加列，只新增 **constraint**）：
...
**归档**：内部讨论 → [discussions/2026-04-25-step-f-scenarios-v2-5expert-review.md](../discussions/2026-04-25-step-f-scenarios-v2-5expert-review.md)。脱敏公开版 → `docs-hub/docs/options-trader/specs/2026-04-25-step-f-execution-wiring-design.md`（本 spec 镜像）+ `docs-hub/docs/options-trader/decisions/2026-04-25-agent2-execution-wiring.md`（客户友好摘要）。
