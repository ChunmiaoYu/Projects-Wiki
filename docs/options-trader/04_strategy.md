...
### 3.1.1 dim 11 长线指标 schema 草样 (21 项 6 family)

**数据来源**: IBKR `reqHistoricalData(durationStr='2 Y', barSizeSetting='1 day')` 拉标的 + SPY 各 500 根 raw 日 bar, 自己用 pandas + numpy + **pandas_ta** + scipy 算 ~10-50 ms / symbol。

按指标族归类 (round 4 D5 锁定, 删 4 项实战不真用项 — A4 支撑/阻力位 / D4 OBV / E5 30d win rate / F5 α):

| Family | 项数 | 字段 |
|---|---|---|
| **A 价格 levels** | 3 | A1 5 时间段 high/low (2y/1y/6m/3m/1m × high/low + 日期 + 成交量) / A2 长期 mean/median/stdev / A3 当前 vs % |
| **B MA trend** | 3 | B1 MA20/50/100/200 / B2 多空排列 + 距 MA200 % / B3 MA50 vs MA200 % + MA200 斜率 |
| **C 波动率** | 4 | C1 RV 30/90/252 / C2 ATR(14) + ATR% / C3 RV 30d 分位 / C4 **HV/IV ratio** (跟 dim 7 联合) |
| **D Volume** | 3 | D1 DAV 2y+30d / D2 30d/2y 比 / D3 当日 vol 分位 |
| **E Momentum** | 4 | E1 1m/3m/6m/12m 涨跌 / E2 RSI(14) / E3 MACD / E4 连续涨跌天数 |
| **F 回撤+相对 SPY** | 4 | F1 MDD 2y+持续+恢复 / F2 当前回撤 / F3 outperform SPY 1m/3m/12m / F4 β 2y |

**Pydantic 模型草样** (final schema 见 spec ship 后真实代码):

```python
class LongHorizonIndicators(BaseModel):
    # A 价格 levels (3)
    price_levels: PriceLevels             # high_low_by_window × 5 时间段, long_term_stats, current_vs_pct
    # B MA trend (3)
    ma_trend: MATrend                     # ma20/50/100/200, alignment, distance_pct
    # C 波动率 (4)
    volatility: Volatility                # rv_30/90/252, atr_14, atr_pct, rv_30d_percentile, hv_iv_ratio
    # D Volume (3)
    volume_stats: VolumeStats             # dav_2y, dav_30d, ratio_30d_2y, today_vol_percentile
    # E Momentum (4)
    momentum: Momentum                    # changes_1m/3m/6m/12m, rsi_14, macd, consecutive_days
    # F 回撤+相对 SPY (4)
    drawdown_relative: DrawdownRelative   # mdd_2y, current_drawdown, outperform_spy, beta_2y
```
