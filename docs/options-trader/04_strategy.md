...
本文档对应 **invariant 10 + 16 + 20 + 21**（项目 `CLAUDE.md §5`）+ 北极星 §1 §2，描述 Agent 2 在新架构（2026-04-21+ 模型锁补丁、2026-04-30 LLM 选型、2026-05-04 / 2026-05-06 / 2026-05-07 / 2026-05-15 系列范式调整）下的真实工作方式。
...
> **2026-05-15 round 4 sync — 3 Agent 体系 + BundleV2 12 维 + entry phase review loop**: 客户开会决定 (D5) 输入 Bundle 从 10 维 → **BundleV2 12 维** (加 dim 11 长线指标 21 项 + dim 12 事件影响); (D6) 加 **Agent 3** (sibling 事件刷新 agent, 维护 dim 12, Agent 2 读 dim 12); (D2) 入场决策 phase 加 **TEMPORARY/PERMANENT/null rejection_type** + entry phase **连续 loop review** (跟事件熔断同 pattern, timeout 60s for entry phase). 详见 §1 / §3.1 / §4.5。Agent 1/2 prompt 不改 (信任 LLM 看数据自决)。
...
Agent 2 在系统里有**两个完全独立但共享 prompt 片段**的工作点。两者用同一份 **BundleV2 12 维** 输入 (2026-05-15 round 4 sync, dim 11 长线指标 + dim 12 事件影响新加, 详见 §3)、同一套风险门校验、同一张 `agent2_decisions` 表，但**触发源、运行节奏、模型选型、输出 schema** 全部不一样：
...
| **触发频率** | 触发瞬间塞 1 次 entry 决策；输出 TEMPORARY rejection → **连续 loop entry review** (跟事件熔断同 pattern, timeout 60s) 直到 PERMANENT / 入场 / 窗口过期 (2026-05-15 round 4 D2 新加, 详见 §4.5) | 每持仓 5 min/次（默认）；事件熔断窗口 → 连续 loop；Adaptive 触发条件命中 → 1-2 min |
| **输入 Bundle** | **BundleV2 12 维** (2026-05-15 round 4 sync)，但 dim 1 持仓盈亏 + dim 8 上次 thesis_summary 必为 null（还没建仓） | **BundleV2 12 维** 完整，含持仓盈亏 + 上次 thesis_summary（rolling ≤600 字） |
| **输出 schema** | 4+1 字段决策：策略类型 + legs 描述（delta_target / expiry_preference）+ quantity + 退出计划（TP/SL/time_stop）+ **`rejection_type` 字段 (TEMPORARY/PERMANENT/null, 2026-05-15 round 4 D2 新加)** | 4 选 1 决策：HOLD / PARTIAL_CLOSE(quantity_pct) / FULL_CLOSE / ADJUST_STOP（XOR 绝对% 或 trailing%）|
...
### 1.4 Agent 3 sibling (2026-05-15 round 4 D6 新加)

Agent 2 不再单打独斗 — **2026-05-15 round 4 客户开会决定加 Agent 3** (3 Agent 体系):

| Agent | 职责 | 触发 | Bundle 维度归属 |
|---|---|---|---|
| Agent 1 (Intake) | 解析客户自然语言机会单 → 派活给 worker | 客户提交机会单 | 不读 BundleV2 |
| Agent 2 (Strategy) | 双工作点决策 (entry + review) | CONDITION_MET / REVIEW_TICK | **读** BundleV2 12 维 (含 dim 12) |
| **Agent 3 (Event Refresh)** | 维护 dim 12 事件影响数据 (per-symbol 机会单+持仓单合集 + macro 子字段) | 开盘瞬间 09:30 ET 立即 1 次 + 盘中每 15 min + 入场前阻塞刷 (鲜度门槛 5 min) + 收盘后停; 盘前/盘外不刷 | **写** dim 12 (Agent 2 只读) |

**关键点 (round 4 D6 锁定)**:
- 新 handler `agent3_event_refresh_handler` + 新 event_type `AGENT3_EVENT_REFRESH_TICK(symbol)` (基础设施 8→9 handler/event_type)
- 本期数据源 = Anthropic SDK web search worker; 远景 NewsCollector
- **Agent 1/2 prompt 不改** (信任 LLM 看 dim 12 数据自决, 不指挥怎么看)
- Agent 3 跟 Agent 2 通过 BundleV2 dim 12 字段间接配合, 无直接调用
...
|---|---|---|
| 入场决策 | LLM 生成 2-3 排序方案 → 前端展示 → **用户必须点确认** → 才执行 | LLM 生成 1 个 4+1 字段决策 (加 `rejection_type` 字段, 2026-05-15 round 4 D2) → 风险门 → 自动执行 / TEMPORARY 留 entry review loop / PERMANENT 立即 FAILED（无用户确认环节）|
...
## 3. BundleV2 12 维输入 — 2026-05-15 round 4 D5 sync

> **2026-05-15 round 4 D5 sync**: BundleV1 (10 维) → **BundleV2 (12 维)**. 加 dim 11 长线指标 21 项 + dim 12 事件影响. dim 1-10 语义不变, 现有 schema/Pydantic 模型/数据采集路径都保留. 来源依据 wiki north-star §5.1.1 + §11 + §16 (single source of truth).

Agent 2 两个工作点的输入永远是同一个 `Agent2DataBundle` Pydantic 模型，**2026-05-15 起永远 12 维 (BundleV2)**。entry 时 dim 1 持仓 + dim 8 news 为 null（无持仓 + 无新闻 collector），review 时维度齐全（除了 news 仍由 NullNewsCollector 返 None，留 Phase 12+ 真接入）。dim 12 由 **Agent 3** 维护写入 (Agent 2 只读, 见 §1.4)。

### 3.1 12 维清单 (2026-05-15 round 4 sync)
...
| **11** | **长线指标 LongHorizonIndicators (2026-05-15 round 4 D5 新加, 21 项 6 family)** | `long_horizon: LongHorizonIndicators` | 见 §3.1.1 详 schema 草样 | 齐 (开盘前预算 ~10-50 ms 算一次, 24h 缓存) |
| **12** | **事件影响 EventImpact (2026-05-15 round 4 D6 新加, Agent 3 维护)** | `event_impact: EventImpact` | per-symbol (机会单+持仓单合集) + macro 子字段, 见 §3.1.2 | 齐 (开盘瞬间 09:30 ET 立即 1 次 + 盘中每 15 min + 入场前阻塞刷 鲜度门槛 5 min) |

**额外字段** `Agent2DataBundle.ages_sec: dict[str, float]` — 每维数据采集到 LLM 调用之间的 stale 时长（秒），review 时给 LLM 看清哪些维度可能 stale。dim 11 由 ages_sec 看 ≤ 24h 即视为 fresh; dim 12 由 ages_sec 看 ≤ 5 min 即视为 fresh (入场前阻塞刷的鲜度门槛).

### 3.1.1 dim 11 长线指标 — 21 项 6 family (D5 PR1+PR2 ship 真实施)

> **2026-05-17 D5 PR3 真数据 sync**: PR1 (alembic 0036 + collectors + LongHorizonIndicators 21 项 Pydantic 模型) + PR2 (cache repo + build_bundle_v2 集成 + SYSTEM_WAKE_UP refresh) 已 ship. 本节字段名/函数名/SQL 跟真代码 `services/long_term_indicators.py` + `services/long_term_stats_cache.py` 一对一。

**为什么要长线指标 (客户痛点本质)**: 10 维 BundleV1 没"长线 perspective" — Agent 2 决策 NVDA LONG_CALL 时, 不知道 NVDA 过去 2 年高点 / 当前 2y 分位 / MA200 斜率上行下行 / 跑赢 SPY 多少, 跟"刚入市散户只看 1m bar 拍脑袋"一样。dim 11 把 500 根 raw 日 bar (~10K tokens) 压成 21 个derived 指标 (~600-700 tokens, 节省 ~95%), Agent 2 一眼看 macro structure.

**数据来源**: IBKR `reqHistoricalData(durationStr='2 Y', barSizeSetting='1 day', whatToShow='TRADES', useRTH=True)` 拉**标的 + SPY 各 ~500 根 raw 日 bar**。Python 自己用 `pandas` + `numpy` + `pandas_ta_classic` + `scipy.stats.linregress` 算 ~10-50 ms / symbol。

按指标族归类 (round 4 D5 锁定, 删 4 项实战不真用项 — A4 支撑/阻力位 / D4 OBV / E5 30d win rate / F5 α):

#### 21 指标完整表 (真字段名)

| Family | # | 字段 (Pydantic 真名) | 计算公式 / 函数 | 实战用途 |
|---|---|---|---|---|
| **A 价格 levels** | A1 | `family_a_price_levels.by_period[2y\|1y\|6m\|3m\|1m].{high,high_date,high_vol,low,low_date,low_vol}` | `window['high'].idxmax()` / `idxmin()`; periods_days = `{2y:504,1y:252,6m:126,3m:63,1m:22}` | 客户原话核心: "至少最高点最低点需要的" |
| | A2 | `family_a_price_levels.long_term_stats.{mean,median,stdev}` | `df['close'].mean()/median()/std()` (2y close) | 当前价 vs 长期均值 → "便宜 / 贵" |
| | A3 | `family_a_price_levels.current_vs_levels.vs_{period}_{high/low/median/mean}_pct` (20 fields) + `current_in_2y_percentile` | `(current-target)/target*100`; percentile = `(window_2y['close'] < current).mean()*100` | LLM 一眼看"当前价处于 2y 71 分位 → 偏高" |
| **B MA trend** | B1 | `family_b_ma_trend.ma_values.{ma20,ma50,ma100,ma200}` (None 若 n < N) | `df['close'].rolling(N).mean().iloc[-1]` | 基础趋势线 |
| | B2 | `family_b_ma_trend.alignment` (enum: `STRONG_BULL\|WEAK_BULL\|MIXED\|WEAK_BEAR\|STRONG_BEAR\|UNAVAILABLE`) | `_classify_ma_alignment(ma20,50,100,200)`: 20>50>100>200 = STRONG_BULL 等 | LLM 一眼看趋势结构 |
| | B3 | `family_b_ma_trend.{distance_to_ma200_pct,ma50_vs_ma200_pct,ma200_slope_20d}` (enum `UP\|FLAT\|DOWN\|UNAVAILABLE`) | slope = `scipy.stats.linregress(range(20), MA200_last_20d).slope`; 相对 ma200 magnitude 0.01% per day 阈值 → UP/DOWN/FLAT | Death Cross / Golden Cross + 长期趋势真伪 |
| **C 波动率** ⭐ 期权实战核心 | C1 | `family_c_volatility.rv_annualized.{30d,90d,252d}` (alias serialization) | `close.pct_change().rolling(N).std() * sqrt(252)` 年化 | 历史波动率三尺度 |
| | C2 | `family_c_volatility.{atr14,atr14_pct}` | `pandas_ta_classic.atr(high,low,close,length=14).iloc[-1]`; atr_pct = atr14 / current_close * 100 | 平均真实波幅 — 期权 strike 选择 + stop 距离参考 |
| | C3 | `family_c_volatility.rv30_percentile_2y` (0-100) | `(rv30_series < rv30).mean() * 100` over 2y RV30 distribution | "现在是高波时段还是低波时段" |
| | C4 ⭐ | `family_c_volatility.{hv_iv_ratio,hv_iv_ratio_interpretation}` (跟 dim 7 联合) | `hv_iv_ratio = rv30 / current_atm_iv` (cache 存 RV30, bundle build 时用实时 dim 7 atm_iv **重算**, 见 `_recompute_hv_iv_ratio`) | **期权 buyer 决策最关键 1 个指标**: ratio > 1 = IV 偏低 (买便宜) / ratio < 1 = IV 偏高 (买贵) / ratio < 0.7 = IV crush 风险 |
| **D Volume** | D1 | `family_d_volume.{dav_2y,dav_30d}` | `int(volume.mean())` (2y) / `int(volume.iloc[-30:].mean())` (30d) | DAV (Daily Avg Volume) 两尺度 |
| | D2 | `family_d_volume.volume_ratio_30d_2y` | `dav_30d / dav_2y` | > 1.2 = 近期 unusual high volume (institutional accumulation 信号) / < 0.8 = 冷淡 |
| | D3 | `family_d_volume.current_day_vol_percentile_2y` (0-100) | `(df['volume'] < current_vol).mean() * 100` | 当日 vol 在 2y daily vol 分布 percentile |
| **E Momentum** | E1 | `family_e_momentum.returns.{1m,3m,6m,12m}_pct` (alias) | `(current/close.iloc[-N-1]-1)*100`; N = 22/66/132/252 trading days | 跨时段动量结构 |
| | E2 | `family_e_momentum.rsi14` | `pandas_ta_classic.rsi(close, length=14).iloc[-1]`; 0-100 | 超买/超卖参考 (50 中性) |
| | E3 | `family_e_momentum.macd_state` (enum: `BULL_CROSS\|BULL_HOLD\|BEAR_CROSS\|BEAR_HOLD\|UNAVAILABLE`) | `pandas_ta_classic.macd(close,12,26,9)` → `_classify_macd_state`: MACD>signal + histogram 上升 = BULL_CROSS | 趋势动量阶段判断 |
| | E4 | `family_e_momentum.consecutive_days` (int: +N 连涨 / -N 连跌) | `_compute_consecutive_days`: 末连续 sign(diff) 计数 × sign | 短期惯性强度 |
| **F 回撤+相对 SPY** | F1 | `family_f_drawdown.{mdd_2y_pct,mdd_duration_days,mdd_recovery_days}` (recovery 可 None) | MDD = `(close / close.cummax() - 1).min() * 100`; duration = peak → trough 日历天; recovery = trough → 新高天数 (None 若未恢复) | 历史最大回撤 + 恢复速度 |
| | F2 | `family_f_drawdown.current_drawdown_pct` | `(close.iloc[-1] / close.cummax().iloc[-1] - 1) * 100` | 当前距 2y rolling peak 多少 % |
| | F3 | `family_f_drawdown.outperform_spy_{1m,3m,12m}_pct` | `sym_ret - spy_ret` (SPY aligned via `reindex(ffill)`); N = 22/66/252 | 相对 SPY 跑赢 / 跑输 (relative strength) |
| | F4 | `family_f_drawdown.beta_2y` | `scipy.stats.linregress(spy_returns, sym_returns).slope` on aligned 2y daily returns (≥30 common days) | 标的相对 SPY 弹性 (期权 sizing 必看: 高 β 同 size = 更大 dollar 风险) |

**顶层元数据** (`LongHorizonIndicators`): `symbol` / `as_of` (UTC) / `bars_used` (实际 IBKR 返多少, IPO < 2y 可能 < 500) / `spy_bars_used` (SPY 用了多少, Family F 用).

#### Family C — HV/IV ratio 深度展开 (⭐ 期权实战核心)

C4 HV/IV ratio 是 dim 11 跟 dim 7 唯一**联合计算**的字段, 由 `services/long_term_stats_cache.py::_recompute_hv_iv_ratio` 在每次 bundle build 时重算:

```python
# 实时 atm_iv 从 dim 7 OptionGreeks.implied_vol 取
def _recompute_hv_iv_ratio(indicators, current_atm_iv):
    rv30 = indicators.family_c_volatility.rv_annualized.d30  # cache 存的 long-term stable 值
    if rv30 is None or current_atm_iv is None or current_atm_iv <= 0:
        new_ratio = None
    else:
        new_ratio = rv30 / current_atm_iv
    new_interpretation = _classify_hv_iv_ratio(new_ratio)
    # Pydantic copy 不变 input
    return indicators.model_copy(update={"family_c_volatility": ...})
```

**Why 重算不直接存**: cache 表存 atm_iv snapshot 仅供 audit (`hv_iv_ratio_atm_iv` 字段), 实际 ratio 必须用 bundle build 当下的实时 atm_iv (期权 IV 分钟级变化, 24h cache 的 IV 值过时无意义).

**中文 interpretation 5 档** (`_classify_hv_iv_ratio`, Agent 2 LLM 直接读不需再算):

| ratio 区间 | interpretation |
|---|---|
| `< 0.7` | "IV 显著偏高 — IV crush 风险 (期权贵)" |
| `[0.7, 0.95)` | "IV 偏高 — 期权当前定价偏贵" |
| `[0.95, 1.05)` | "IV 跟 HV 持平 — 期权定价合理" |
| `[1.05, 1.3)` | "IV 略偏低 — 期权当前定价偏便宜" |
| `>= 1.3` | "IV 显著偏低 — 期权当前定价便宜 (重要: 波动率反转风险)" |
| `None` (无 IV) | "IV data unavailable — 无法判断期权贵不贵" |

#### Family A — 5 时段 high/low + 当前 vs % 表 (客户原话核心需求)

5 时间段 (2y/1y/6m/3m/1m) 取 trading-day 窗口 `{504, 252, 126, 63, 22}`. 每时段返 high + high_date + high_vol + low + low_date + low_vol, **当前 close vs 各 high/low/median/mean** 算 % 差距. 总共 20 个 `vs_*_pct` 字段 + 1 个 `current_in_2y_percentile` (0-100).

JSON 输出样:
```json
{
  "by_period": {
    "2y": {"high": 312.45, "high_date": "20260102", "high_vol": 89234567,
           "low": 175.20, "low_date": "20250414", "low_vol": 52341890},
    "1y": {...}, "6m": {...}, "3m": {...}, "1m": {...}
  },
  "long_term_stats": {"mean": 245.32, "median": 248.10, "stdev": 28.45},
  "current_vs_levels": {
    "vs_2y_high_pct": -9.12, "vs_2y_low_pct": 62.18,
    "vs_2y_median_pct": 14.45, "vs_2y_mean_pct": 15.62,
    "current_in_2y_percentile": 71.4
  }
}
```

#### 真 Pydantic schema (`services/long_term_indicators.py`)

```python
class LongHorizonIndicators(BaseModel):
    """BundleV2 dim 11 — 长线指标 21 项 (6 family)."""
    symbol: str
    as_of: datetime
    bars_used: int                       # 实际 IBKR 返多少根 (期望 ~500, IPO 不到 2y 会少)
    spy_bars_used: int

    family_a_price_levels: FamilyA_PriceLevels
    family_b_ma_trend: FamilyB_MATrend
    family_c_volatility: FamilyC_Volatility
    family_d_volume: FamilyD_Volume
    family_e_momentum: FamilyE_Momentum
    family_f_drawdown: FamilyF_Drawdown
```

#### IPO < 2y 标的容错

返 `bars_used < 500` (e.g. 100), pandas `rolling(200)` 前 199 天 NaN → `MAValues.ma200=None` / `alignment=UNAVAILABLE` / `ma200_slope_20d=UNAVAILABLE` / `returns.12m_pct=None` / `beta_2y=None`. Agent 2 prompt 看 None 字段时按"数据不足"分支处理 (跟 dim 8 NullNewsCollector 同模式).

#### Cache + 计算时机

详 [[依赖项与配置|08_dependencies.md]] §11 (依赖) + 本节 §3.3 (调用入口 `build_bundle_v2`).

| 触发 | 行为 |
|---|---|
| 入场前 cache hit (`as_of >= today ET`) | 直接读 `long_term_stats_cache.indicators_json` + 用实时 atm_iv 重算 HV/IV ratio (~10 ms) |
| 入场前 cache miss / stale | 阻塞 `fetch_long_term_bars(symbol)` + `fetch_long_term_bars('SPY')` (~6-10 s 双 IBKR query) + 计算 21 指标 (~10-50 ms) + upsert cache |
| `SYSTEM_WAKE_UP` (每日开盘前) | 后台异步重算全 active opps + holdings symbols (~35 symbol × 0.5 s sleep ≈ 17 s, 不阻塞主路径) |
| Manual force refresh | `LongTermStatsCacheRepository.force_invalidate(symbol)` 删 cache 行, 下次 bundle build 走 miss 路径重算 |
| 5 min review (持仓) | **不重算** (cache 命中即可); 日级别指标 1 天内重算无意义, 节省 IBKR quota |

### 3.1.2 dim 12 事件影响 schema 草样 (Agent 3 维护)

per-symbol 机会单+持仓单合集 (Agent 3 看用户所有 active opp + position 涉及的 symbol 全集) + macro 子字段:

```python
class EventImpact(BaseModel):
    # per-symbol (持仓+机会单合集涉及的 symbols)
    per_symbol_events: dict[str, list[SymbolEvent]]  # AAPL: [earnings, FDA approval, ...]
    # macro 子字段 (跨 symbol 共同)
    macro_events: list[MacroEvent]                    # FOMC, CPI, NFP, GDP, ...
    last_refreshed_at_utc: datetime
    refresh_source: Literal["web_search", "news_collector"]  # 本期 web_search; 远景 news_collector
```

Agent 2 看 dim 12 字段自决 (信任 LLM, 不在 prompt 写"如果 FOMC 在 30 min 内则..." 类指令)。

### 3.2 entry vs review 的差异 (2026-05-15 round 4 sync: 12 维统一)
...
entry 路径 (strategy_run_id is None):
  ...
  - long_horizon = 齐 (dim 11, 24h 缓存)
  - event_impact = 齐 (dim 12, Agent 3 维护, 入场前阻塞刷鲜度 ≤ 5 min)
...
review 路径 (strategy_run_id is not None):
  ...
  - news = None (同上)
  - long_horizon = 齐 (dim 11, 24h 缓存)
  - event_impact = 齐 (dim 12, 盘中每 15 min refresh)
```
...
### 3.3 数据采集 — `pack_bundle` (V1) + `build_bundle_v2` (V2, dim 11+12 集成)

**V1 入口** (`pack_bundle`, sync) 组合 dim 1-10:
...
**V2 入口** (`build_bundle_v2`, async, 2026-05-15 D5 PR2 ship) 包裹 V1 + 加 dim 11 长线指标 + dim 12 事件影响:

```python
from options_event_trader.services.data_collection.bundle_packager import build_bundle_v2

bundle_v2: Agent2DataBundleV2 = await build_bundle_v2(
    opportunity_id=opportunity_id,
    strategy_run_id=strategy_run_id,
    db_reader=db_reader,
    ibkr_client=ibkr_client,            # 同 V1, 但 dim 11 需要 req_historical_bars 协程方法
    subscription_manager=subscription_manager,
    db_session=db_session,              # **V2 新增**: dim 11 cache 表 read/write 必需
    external_collector=external_collector,
    news=news_snapshot,
    account_snapshot_client=account_snapshot_client,
)
# bundle_v2.bundle_version == "v2"
# bundle_v2.long_horizon_indicators: LongHorizonIndicators | None   (dim 11, 失败 None graceful)
# bundle_v2.event_impact: EventImpact | None                        (dim 12, Agent 3 写, packager 读)
```

**V2 流程**:
1. 调 `pack_bundle(...)` 组 dim 1-10 (V1 unchanged)
2. 从 V1.greeks.implied_vol 取 live atm_iv (entry 路径 None ok)
3. await `get_or_compute_long_term_indicators(symbol, db, ibkr_client, current_atm_iv)` 拿 dim 11 (cache hit 走快路径 / miss 双 IBKR query + 计算 + upsert / 异常 graceful None)
4. await `build_dim12_from_snapshots(db_session, symbol=...)` 读 Agent 3 写的最新事件 snapshot (失败 None graceful)
5. compose `Agent2DataBundleV2(bundle_version="v2", ..., long_horizon_indicators=dim11, event_impact=dim12)`

**Backward compat**: V1 `pack_bundle` unchanged. Cutover 留 1 周 paper 观察 (finding F-2026-05-22-BUNDLE-V1-BUILDER-DEAD-CODE-CLEANUP).

`pack_bundle` / `build_bundle_v2` **都不直接调 IBKR API** — 走 4 个 Pool/Client 抽象（MarketDataPool / QueryClient / OrderClient / AccountSnapshotClient）。这是北极星 §5 forward compat hook："业务代码必走 4 个 Pool/Client 抽象，禁止直接调 `ibkr.reqMktData / reqPositions / reqAccountSummary / placeOrder`。"
...
| rejection_type | 含义 | 下游动作 |
|---|---|---|
| `null` (no rejection) | LLM 决定进场, 4+1 字段决策正常 | 风险门 2 层 → server-compile → 入场单 |
| `TEMPORARY` | "现在不好但条件可能改善" (e.g. IV 太高等回落 / 流动性当下不够 / 价格刚突破想等回踩) | **留 entry review loop, 连续重跑** (跟事件熔断同 pattern, timeout 60s/轮) 直到 PERMANENT / 入场 / 窗口过期 |
| `PERMANENT` | "结构性不进场" (e.g. 标的处于停盘 / 客户原话指定方向跟 dim 11 长线趋势严重矛盾 / Agent 2 多次重试仍 confidence < 阈值) | **立即 FAILED**, opp.status = FAILED (用户可见, 决策时间线显示 PERMANENT rejection 原因) |

#### 4.5.2 复用 `active_reviews` 表 — `review_phase` ENUM 列 (round 4 D2 schema)

**round 4 D2 决定**: 不新建 entry-only 表, **复用 `active_reviews` 表**加 `review_phase` ENUM 列区分:

```sql
-- alembic 0030+ (2026-05-15 round 4 D2 schema, 待 ship)
ALTER TABLE active_reviews
  ADD COLUMN review_phase VARCHAR(16) NOT NULL DEFAULT 'POSITION'
    CHECK (review_phase IN ('ENTRY', 'POSITION'));

-- entry phase 触发条件命中时 INSERT (review_phase='ENTRY')
-- entry phase 入场成功 → UPDATE review_phase='POSITION' (continues normal 5 min review)
-- entry phase PERMANENT / 窗口过期 → DELETE
```

#### 4.5.3 entry review loop 流程图

```mermaid
flowchart TD
    A[CONDITION_MET 触发<br/>active_reviews INSERT review_phase=ENTRY]
    A --> B[Agent 2 entry 决策<br/>timeout 60s/轮]
    B --> C{rejection_type}
    C -- null<br/>风险门通过 --> D[入场单<br/>UPDATE review_phase=POSITION]
    C -- TEMPORARY --> E{窗口未过期?}
    E -- yes --> F[连续 loop 下一轮<br/>同 pattern 事件熔断]
    F --> B
    E -- no --> G[FAILED 窗口过期]
    C -- PERMANENT --> H[立即 FAILED]
    D --> I[normal review 5 min 节奏]
```

#### 4.5.4 跟事件熔断窗口对比

| 维度 | 事件熔断窗口 | Entry phase review loop (round 4 D2) |
|---|---|---|
| **触发** | newsTicker 收事件 / 财报/CPI/FOMC ±15 min | CONDITION_MET 触发条件成真但 Agent 2 输出 TEMPORARY |
| **节奏** | 连续 loop (interval=1s floor) | 连续 loop (跟事件熔断同 pattern) |
| **单次 timeout** | 25s | **60s for entry phase** (entry 决策思考量大, 给 60s vs review 25s) |
| **退出条件** | 窗口 60 min 过期 / 事件影响消化 | 入场 (null rejection) / PERMANENT / 窗口过期 |
| **schema 存储** | `active_reviews.review_interval_sec` 字段 | `active_reviews.review_phase='ENTRY'` 区分 |

**为什么连续 loop 不写死 interval**: 跟事件熔断同理 (北极星 §7 反偏离) — LLM entry 决策延迟 ~5-30s, 写死 interval 等于上一轮没出结果下一轮又开始, 重叠堆栈反而不稳。连续 loop = 上一轮 finalize 后立即下一轮 (floor 由 timeout 60s 自然托底)。

Sources: [services/review_scheduler.py L1-L53](https://github.com/ChunmiaoYu/options_ai_trader/blob/6b3d159/src/options_event_trader/services/review_scheduler.py#L1-L53), 北极星 §1（事件熔断窗口）, specs/north-star-v1-target.md §5.1.1 + §11 + §16 (round 4 D2 决定 entry phase review loop), specs/architecture-walkthrough §6（5 min review 循环）
...
2. **LLM 看 BundleV2 12 维能判断吗?** 不能；裸卖空的"市场极端波动" 在 bundle 里看不出，bundle 里的 IV / 流动性 / OI / dim 11 长线波动率 / dim 12 事件影响 都跟"账户能不能扛得住极端事件"无关
...
| LLM 看 BundleV2 12 维能判断吗? | **能**（dim 6 OptionLiquidity 含 OI / day_volume / spread_pct，dim 9 UnusualActivity）→ soft hint |
...
agent2_decisions
  ...
  bundle_json              JSONB NOT NULL          -- BundleV2 12 维 bundle 完整 dump (2026-05-15 round 4 D5)
...
| 止损时机 | 绝不让用户定死止损价位 | LLM 综合 dim 1-12 (BundleV2, 含 dim 11 长线指标 + dim 12 事件影响) 决定 PARTIAL/FULL_CLOSE / ADJUST_STOP，不读用户 UI 上的 stop_loss 输入框（前端的 SL 字段 v3 后已删，invariant 5 v3）|
...
flowchart TB
    ...
    subgraph BundlePack["3. 拉 BundleV2 12 维 Bundle (2026-05-15 round 4)"]
        BP1[pack_bundle]
...
- **首次入场（工作点 A）** — DeepSeek V4-Flash 看 **BundleV2 12 维** (2026-05-15 round 4 D5 sync, 加 dim 11 长线指标 21 项 + dim 12 事件影响 Agent 3 维护) 选策略 + 决定 quantity/TP/SL + `rejection_type` 字段 (TEMPORARY/PERMANENT/null, round 4 D2)，TEMPORARY 留 entry phase review loop 连续重跑 (timeout 60s, round 4 D2)，风险门 2 层校验通过自动入场
...
- **BundleV2 dim 11 长线指标 21 项 schema 变化** (2026-05-15 round 4 D5) → 同步 §3.1.1 + 真实代码 `domain/agent2_bundle.py::LongHorizonIndicators`
- **dim 12 事件影响 schema 变化** (2026-05-15 round 4 D6) → 同步 §3.1.2 + `domain/agent2_bundle.py::EventImpact` + Agent 3 handler `agent3_event_refresh_handler` + event_type `AGENT3_EVENT_REFRESH_TICK`
- **Entry phase review loop schema 变化** (2026-05-15 round 4 D2) → 同步 §4.5 + `active_reviews.review_phase` ENUM + `Agent2EntryDecision.rejection_type` 字段
- **Agent 3 职责变化** (round 4 D6) → 同步 §1.4 + Agent 3 spec ship 后真实代码路径
