...
| 2026-05-17 晚 | **Pipeline 状态 UI 完善 + 4 轮 label/handler 修真 bug** | V1 ship 完用户在 admin → Pipeline tab 发现 (a) **只显示 10 行 (cron 每小时跑只够 10 小时)** → 改 `LIMIT 10` → `WHERE started_at >= 7 days` (4d868b6); (b) 7 天 133 行不直观, 用户提议 "1 天 1 页 + 跳过空日" → 加 NZ 时区分组分页, prev/next + select dropdown 显示每天行数 (0c8e736); (c) **label/behavior 4 轮 round-trip 修真 bug**: label 我从第 2 轮 52e82a9 就对了 但 idx++/-- 一直没 swap, 真 bug 在 handler 不在文字 (commit 54b2be1 swap idx + tooltip 加 DESC 解释). **教训**: 用户说 "swap 位置" UI 控件时, label 改 + handler 行为必须同时改, 不只 label. **V1 实质达成**, 用户明示 "V2 没想好, 现在最多 V1 缝缝补补", session 结束 |
